...
export async function GET(req: Request, props: { params: Promise<{ id: string }> }) {
    ...
    try {
        await dbConnect();
        const item = await Tranche.findById(params.id).populate("frais");
...
export async function PUT(req: Request, props: { params: Promise<{ id: string }> }) {
    ...
    try {
        ...
        const item = await Tranche.findByIdAndUpdate(params.id, body, { new: true });
        if (!item) return NextResponse.json({ success: false, message: "Non trouvé" }, { status: 404 });
...
export async function DELETE(req: Request, props: { params: Promise<{ id: string }> }) {
    ...
    try {
        await dbConnect();
        const item = await Tranche.findByIdAndDelete(params.id);
        if (!item) return NextResponse.json({ success: false, message: "Non trouvé" }, { status: 404 });
